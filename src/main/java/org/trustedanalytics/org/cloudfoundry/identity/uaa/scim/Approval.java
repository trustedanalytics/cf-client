...
public class Approval {
    ...
    public Approval(String userId, String clientId, String scope, int expiresIn, ApprovalStatus status) {
        ...
        expiresTime.add(Calendar.MILLISECOND, expiresIn);
        setExpiresAt(expiresTime.getTime());
    }
    ...
    public void setExpiresAt(Date expiresTime) {
        if (expiresTime == null) {
            Calendar thirtyMinFromNow = Calendar.getInstance();
            thirtyMinFromNow.add(Calendar.MINUTE, 30);
            expiresTime = thirtyMinFromNow.getTime();
        }
        this.expiresAt = expiresTime;
    }
